...
class KANSR:
    ...
    def get_symbolic(self, population=10, generations=3, temperature=0.1, exit_condition=1e-3):
        ...
        for i, expr in enumerate(full_expression):
            ...
            try:
                params_opt, chi_squared = self.fit_curve(dataset['train_input'].numpy().flatten(), dataset['train_label'].numpy().flatten(), curve, params_initial)
                self.logger.info(f"Refitting after simplifying gave a chi^2 of {chi_squared:.4e}")
            ...
            try:
                ...
                params_opt, chi_squared = self.fit_curve(dataset['train_input'].numpy().flatten(), dataset['train_label'].numpy().flatten(), curve, params_initial)
                self.logger.info(f"Refitting after final global simplify gave a chi^2 of {chi_squared:.4e}")    
                expr = self.simplify_expression(self.subst_params(curve_ansatz_str, params_opt), self.model.width_in[0] - 1)
